...
def get_catchement():
    ...
    # Alternatively, there is another approach (not shown here) that calculates the 
    # travel times for each raster cell to the closest train station. However, this alternative 
    # has some bugs in its implementation but could be further developed if the analysis 
    ...
    e_min = limits_corridor[0]  # Easting minimum
    n_min = limits_corridor[1]  # Northing minimum
    e_max = limits_corridor[2]  # Easting maximum
    n_max = limits_corridor[3]  # Northing maximum
    ...
    total_times_within_buffer = calculate_total_travel_time(bus_times_within_buffer, closest_trainstations_df)


    # correct the ID of the trainstations to the official ID fo each trainstop (-->NR from the rail_node file)

    rail_node = pd.read_csv(r"data/Network/Rail_Node.csv", encoding='latin1', sep=';')

    # Step 1: Merge the stops and rail_node DataFrames
    merged_stops = pd.merge(stops, rail_node, left_on='CHSTNAME', right_on='NAME', how='inner')
    # Create a mapping from DIVA_NR to NR
    diva_to_nr = merged_stops.set_index('DIVA_NR')['NR']
    # Replace DIVA_NR in 'closest_train_station' with corresponding NR
    total_times_within_buffer['closest_train_station'] = total_times_within_buffer['closest_train_station'].map(diva_to_nr)
    ...
    # Step 1 Load the catchment raster data
    ...
    for geom, value in shapes(catchment_tif, transform=catchment_transform):
        if value != 0:  # Exclude background (0) if necessary
            polygons.append(shape(geom))
            values.append(value)

    # Step 3: Create a GeoDataFrame
    gdf = gpd.GeoDataFrame({"id": values, "geometry": polygons}, crs='EPSG:2056')

    # Step 4: Save as GeoPackage
    gdf.to_file(r"data/catchment_pt/catchement.gpkg", driver="GPKG")
    ...
    #output_path = r"data/catchment_pt"
    #process_and_save(total_times_within_buffer, output_path)
...
with rasterio.open('output.tif') as src:
    # Read the first band (total_time)
    total_time_raster = src.read(1)
    # Read the second band (closest_train_station)
    station_raster = src.read(2)
    # Get the extent for proper plotting
    extent = (src.bounds.left, src.bounds.right, src.bounds.bottom, src.bounds.top)

# Plot the first band (total_time_raster)
plt.figure(figsize=(10, 6))
plt.title('Total Time Raster')
plt.imshow(total_time_raster, cmap='viridis', extent=extent)
plt.colorbar(label='Total Time')
plt.xlabel('Longitude')
plt.ylabel('Latitude')
plt.grid(False)
plt.show()

# Plot the second band (station_raster)
plt.figure(figsize=(10, 6))
plt.title('Closest Train Station Raster')
plt.imshow(station_raster, cmap='plasma', extent=extent)
plt.colorbar(label='Closest Train Station')
plt.xlabel('Longitude')
plt.ylabel('Latitude')
plt.grid(False)
plt.show()
