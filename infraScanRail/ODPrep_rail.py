...
def GetTAZwithCommunes(basis):
    ...
    # Filter out rows with Empty Polygon or None geometry values
    basis = basis[~basis.geometry.isna()]
    basis = basis[~basis.geometry.is_empty]
    basis = basis[~basis.geometry.apply(lambda geom: isinstance(geom, Polygon) and geom.equals(Polygon()))]
