...
def import_data(limits):
    ...
    betriebzaehlung20 = pd.read_csv(
        r"data/independent_variable/statent/ag-b-00.03-22-STATENT2020/STATENT_2020.csv", sep=";"
    )
    # BXXS2: Arbeitsstätte Sektor 2, BxxVZAS2: Vollzeitäquivalent Sektor 2
    betriebzaehlung20 = betriebzaehlung20[["B08VZAT", "E_KOORD", "N_KOORD"]].rename({"B08VZAT": "empl20"}, axis=1)
    ...
    empl20 = empl20_ch[
        (empl20_ch["E_COORD"] >= limits[0])
        & (empl20_ch["E_COORD"] <= limits[2] - 100)
        & (empl20_ch["N_COORD"] >= limits[1])
        & (empl20_ch["N_COORD"] <= limits[3] - 100)
    ]
    ...
    pop20 = pop20_ch[
        (pop20_ch["E_COORD"] >= limits[0])
        & (pop20_ch["E_COORD"] <= limits[2] - 100)
        & (pop20_ch["N_COORD"] >= limits[1])
...
def csv_to_tiff(data_table, attribute, path, rastersize=100):
    ...
    with rasterio.open(
        ...
        height=height,
        count=1,
        dtype=data_table_sorted[attribute].dtype,
        crs=crs.CRS.from_string(crs_value),
        transform=transform,
    ) as dst:
...
def import_locations():
    ...
    gdf_cities = gpd.GeoDataFrame(
        df_cities, geometry=gpd.points_from_xy(df_cities["x"], df_cities["y"]), crs="epsg:2056"
    )
...
def load_nw():
    ...
    edge_table = pd.merge(
        edge_table,
        link_attribute[["Link Typ", "NAME", "Rank", "Lanes", "Capacity", "Free Flow Speed"]],
        how="left",
        on="Link Typ",
    )
    ...
    edge_table = edge_table[
        (edge_table["Rank"] == 1)
        & (edge_table["Opening Year"] < 2023)
        & (edge_table["NAME"] != "Freeway Tunnel planned")
...
def map_access_points_on_network(current_points, network):
    ...
    current_points = gpd.GeoDataFrame(
        current_points, geometry=gpd.points_from_xy(current_points["x"], current_points["y"]), crs="epsg:2056"
    )
    ...
    for index, point in current_points.iterrows():
        ...
        if network.distance(point.geometry).min() < 300:
            ...
            adjusted_points_gdf = gpd.GeoDataFrame(
                pd.concat([pd.DataFrame(adjusted_points_gdf), pd.DataFrame(point).T], ignore_index=True)
            )
...
def reformat_network():
    ...
    edges_gdf["E_KOORD_O"], edges_gdf["N_KOORD_O"], edges_gdf["E_KOORD_D"], edges_gdf["N_KOORD_D"] = np.where(
        edges_gdf["x_origin"] < edges_gdf["x_dest"],
    ...
    for coord in unique_coords:
        count = ((edges_gdf["x_origin"] == coord[0]) & (edges_gdf["y_origin"] == coord[1])).sum() + (
            (edges_gdf["x_dest"] == coord[0]) & (edges_gdf["y_dest"] == coord[1])
        ).sum()
    ...
    overlay_f = gpd.sjoin(
        df_first, crossing_nodes, how="left", predicate="intersects", lsuffix="first", rsuffix="geometry"
    )
    ...
    overlay_l = gpd.sjoin(
        df_last, crossing_nodes, how="left", predicate="intersects", lsuffix="last", rsuffix="geometry"
    )
    ...
    endpoints = [Point(edge.coords[0]) for edge in filtered_edges.geometry] + [
        Point(edge.coords[-1]) for edge in filtered_edges.geometry
    ]
    ...
    points_completed = gpd.GeoDataFrame(
        pd.concat([pd.DataFrame(points_gdf), pd.DataFrame(missing_points)], ignore_index=True)
    )
...
def required_manipulations_on_network():
    ...
    points = gpd.GeoDataFrame(
        pd.concat([pd.DataFrame(points), pd.DataFrame(pd.Series(new_row_data)).T], ignore_index=True)
    )
    ...
    for start_id, end_id in pairs_to_connect:
        ...
        edges = gpd.GeoDataFrame(
            pd.concat([pd.DataFrame(edges), pd.DataFrame(pd.Series(new_row)).T], ignore_index=True)
        )
...
def get_edge_attributes():
    ...
    edges_process["capacity"], edges_process["ffs"] = zip(
        *edges_process.apply(lambda x: get_min_attributes(x, edges_raw), axis=1)
    )
    ...
    logger.verbose(edges_process[edges_process["ffs"].isnull()])
...
def network_in_corridor(polygon):
    ...
    temp_joined = gpd.sjoin(
        points_temp, edges_crossing_polygon, how="left", predicate="intersects", lsuffix="left", rsuffix="right"
    )
...
def map_values_to_nodes():
    ...
    points_raw = gpd.GeoDataFrame(
        points_raw, geometry=gpd.points_from_xy(points_raw["x"], points_raw["y"]), crs="epsg:2056"
    )
...
def only_links_to_corridor():
    all_links = gpd.read_file(r"data\Network\processed\new_links_new_access.gpkg")
    ...
    logger.verbose(access_corridor.head(50).to_string())
    ...
    links_corridor = all_links.merge(
        right=access_corridor[["cor_1", "ID_point"]], left_on="ID_current", right_index=True
    )  # right_on="ID_point")
    logger.verbose(
        "Links connected to access points within the corridor: ", links_corridor.shape[0], " of ", all_links.shape[0]
    )
    ...
    generated_points = gpd.read_file(r"data\Network\processed\generated_nodes_new_access.gpkg")
    ...
    gpd.GeoDataFrame(generated_points_corridor, geometry="geometry_x").to_file(
        r"data\Network\processed\generated_nodes_connecting_corridor.gpkg"
    )
    # gpd.GeoDataFrame(all_objects_corridor,geometry="geometry_x").to_file(r"data\Network\processed\generated_objects_connecting_corridor.shp")


def get_protected_area(limits):
    bln = gpd.read_file(
        r"data\landuse_landcover\Schutzzonen\BLN\N2017_Revision_landschaftnaturdenkmal_20170727_20221110.shp"
    )
    ...
    trockenlandschaften = gpd.read_file(
        r"data\landuse_landcover\Schutzzonen\Moorlandschaft\Moorlandschaft_LV95\moorlandschaft.shp"
    )
    ...
    bundesinventar_auen = gpd.read_file(
        r"data\landuse_landcover\Schutzzonen\Bundesinventar_auen\N2017_Revision_Auengebiete_20171101_20221122.shp"
    )
    ...
    gdf_partly_protected = [wald, fruchtfolgeflaeche, trockenweiden, trockenlandschaften]
    names_partly_protected = ["wald", "fruchtfolgeflaeche", "trockenweiden", "trockenlandschaften"]
...
def multiple_shp_to_one(gdf_list, names_list, path, limits):
    ...
    combined_gdf.to_file(rf"data\landuse_landcover\Schutzzonen\{path}.gpkg", driver="GPKG")
    ...
    combined_gdf_frame.to_file(rf"data\landuse_landcover\processed\{path}_frame.gpkg")
...
def all_protected_area_to_raster(suffix=""):
    ...
    with rasterio.open(tif_file) as src:
        ...
        with rasterio.open(
            rf"data\landuse_landcover\processed\zone_no_infra\protected_area_{suffix}.tif", "w", **meta
        ) as dst:
...
def landuse(limits):
    ...
    areal_stat = pd.read_csv(r"data/landuse_landcover/landcover/ag-b-00.03-37-area-csv.csv", sep=";")
    if limits:
        areal_stat = areal_stat[
            (areal_stat["E_COORD"] >= limits[0])
    ...
    protected_categories = [1, 2, 3, 4, 5, 7, 8, 9, 10, 17, 18, 23, 27]
...
def get_unproductive_area(limits):
    ...
    csv_to_tiff(
        unproductive_area_full, attribute="AS18_27", path=r"data\landuse_landcover\processed\unproductive_area.tif"
    )
    ...
    for geom, value in shape_gen:
        if value == 1:  # Assuming '1' corresponds to the shapes you want
            ...
            geometries = gpd.GeoDataFrame(
                pd.concat([pd.DataFrame(geometries), pd.DataFrame(pd.Series(tempgeom)).T], ignore_index=True)
            )
