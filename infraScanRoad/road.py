...
class Road:
    ...
    def __init__(self, config: dict):
        ...
        self.wd = os.path.join(
            self.config["General"].get("working_directory", ""), "infraScanRoad"
        )  # Working Directory
    ...
    def infrastructure_network_generate_development(self):
        ...
        if bool_new_access:
            ...
            random_gdf = road_generate_infrastructure.generated_access_points(
                extent=self.innerboundary, number=self.n_new_access_points
            )
        ...
        if bool_connecting_roads:
            ...
            # Generate connecting roads between the existing highway network
            road_generate_infrastructure.generate_new_connections_between_hwy(
                self.limits_corridor,
    ...
    def compute_construction_cost(self):
        ...
        road_scoring.construction_costs(
            highway=self.c_openhighway,
            tunnel=self.c_tunnel,
            bridge=self.c_bridge,
            ramp=self.ramp,
        )
        road_scoring.maintenance_costs(
            duration=self.maintenance_duration,
            highway=self.c_om_openhighway,
            tunnel=self.c_om_tunnel,
            bridge=self.c_om_bridge,
            structural=self.c_structural_maint,
        )
    ...
    def compute_cost_externalities(self):
        ...
        road_scoring.externalities_costs(
            ce_highway=self.co2_highway,
            ce_tunnel=self.co2_tunnel,
            realloc_forest=self.forest_reallocation,
            realloc_FFF=self.meadow_reallocation,
            realloc_dry_meadow=self.meadow_reallocation,
            realloc_period=self.reallocation_duration,
            nat_fragmentation=self.fragmentation,
            fragm_period=self.fragmentation_duration,
            nat_loss_habitat=self.habitat_loss,
            habitat_period=self.habitat_loss_duration,
        )

        logger.road(" -> Noise")
        road_scoring.noise_costs(
            years=self.noise_duration,
            boundaries=self.noise_distance,
            unit_costs=self.noise_values,
        )
    ...
    def compute_accessiblity_cost(self):
        ...
        road_scoring.accessibility_developments(
            accessib_status_quo, VTT_h=self.VTTS, duration=self.travel_time_duration
    ...
    def visualize_results(self):
        ...
        road_plots.plot_voronoi_development(
            ...
            voronoi_dev_2,
            generated_points,
            boundary=self.innerboundary,
            network=self.network,
            access_points=self.current_access_points,
        ...
        road_plots.plot_single_cost_result(
            df_costs=self.gdf_costs,
            banned_area=tif_path_plot,
